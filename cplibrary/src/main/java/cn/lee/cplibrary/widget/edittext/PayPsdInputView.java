...
public class PayPsdInputView extends android.support.v7.widget.AppCompatEditText {
    ...
    private void getAtt(AttributeSet attrs) {
        TypedArray typedArray = mContext.obtainStyledAttributes(attrs, R.styleable.cp_PayPsdInputView);
        maxCount = typedArray.getInt(R.styleable.cp_PayPsdInputView_cp_maxCount, maxCount);
        circleColor = typedArray.getColor(R.styleable.cp_PayPsdInputView_cp_circleColor, circleColor);
        bottomLineColor = typedArray.getColor(R.styleable.cp_PayPsdInputView_cp_bottomLineColor, bottomLineColor);
        radius = typedArray.getDimensionPixelOffset(R.styleable.cp_PayPsdInputView_cp_radius, radius);

        divideLineWidth = typedArray.getDimensionPixelSize(R.styleable.cp_PayPsdInputView_cp_divideLineWidth, divideLineWidth);
        divideLineColor = typedArray.getColor(R.styleable.cp_PayPsdInputView_cp_divideLineColor, divideLineColor);
        psdType = typedArray.getInt(R.styleable.cp_PayPsdInputView_cp_psdType, psdType);
        rectAngle = typedArray.getDimensionPixelOffset(R.styleable.cp_PayPsdInputView_cp_rectAngle, rectAngle);
        focusedColor = typedArray.getColor(R.styleable.cp_PayPsdInputView_cp_focusedColor, focusedColor);
...
//       super.onDraw(canvas);
    ...
    public interface onPasswordListener {
        void onDifference(String oldPsd, String newPsd); //和上次输入的密码不一致
        void onEqual(String psd); //两次输入密码相同
        void inputFinished(String inputPsd);  //输完逻辑
    }
}
