package cn.lee.cplibrary.widget.easyswipemenulibrary;
...
import static cn.lee.cplibrary.widget.easyswipemenulibrary.EasySwipeMenuLayout.State.CLOSE;
 ...
 * 使用方式：
 * 1、布局文件中引用：必须要设置3个属性
 *  app:cp_contentView="@+id/content"
 *  app:cp_leftMenuView="@+id/left"
 *  app:cp_rightMenuView="@+id/right"
 *  并且不能用@id/方式，子布局按照left、content、right的顺序方式展示
 *  2、easySwipeMenuLayout.resetStatus(); 恢复只展示cp_contentView的状态，
 *  3、具体使用方式见：cn.com.huoyibang_driver.ui.adapter.feedback.FeedbackHistoryRecyclerViewAdapter
 */
...
public class EasySwipeMenuLayout extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //1、获取配置的属性值
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.cp_EasySwipeMenuLayout, defStyleAttr, 0);

        try {
            int indexCount = typedArray.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int attr = typedArray.getIndex(i);
                if (attr == R.styleable.cp_EasySwipeMenuLayout_cp_leftMenuView) {
                    mLeftViewResID = typedArray.getResourceId(R.styleable.cp_EasySwipeMenuLayout_cp_leftMenuView, -1);
                } else if (attr == R.styleable.cp_EasySwipeMenuLayout_cp_rightMenuView) {
                    mRightViewResID = typedArray.getResourceId(R.styleable.cp_EasySwipeMenuLayout_cp_rightMenuView, -1);
                } else if (attr == R.styleable.cp_EasySwipeMenuLayout_cp_contentView) {
                    mContentViewResID = typedArray.getResourceId(R.styleable.cp_EasySwipeMenuLayout_cp_contentView, -1);
                } else if (attr == R.styleable.cp_EasySwipeMenuLayout_cp_canLeftSwipe) {
                    mCanLeftSwipe = typedArray.getBoolean(R.styleable.cp_EasySwipeMenuLayout_cp_canLeftSwipe, true);
                } else if (attr == R.styleable.cp_EasySwipeMenuLayout_cp_canRightSwipe) {
                    mCanRightSwipe = typedArray.getBoolean(R.styleable.cp_EasySwipeMenuLayout_cp_canRightSwipe, true);
                } else if (attr == R.styleable.cp_EasySwipeMenuLayout_cp_fraction) {
                    mFraction = typedArray.getFloat(R.styleable.cp_EasySwipeMenuLayout_cp_fraction, 0.5f);
                }
            }
