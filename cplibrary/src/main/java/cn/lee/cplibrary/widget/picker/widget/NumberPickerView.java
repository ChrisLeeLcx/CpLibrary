...
public class NumberPickerView extends View {
    ...
    // default  backgroundColor of text's selected
    private static final int DEFAULT_BG_SELECTED_COLOR = 0XFFF5F5F9;
    ...
    private static final boolean DEFAULT_SHOW_TEXT_BG_SELECTED = false;
    ...
    private int mTextBgColorSelected = DEFAULT_BG_SELECTED_COLOR;
    ...
    //true to show the text selected background
    private boolean mShowTextBgSelected = DEFAULT_SHOW_TEXT_BG_SELECTED;
    ...
    private Paint mPaintTextSelectedBg = new Paint();
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.cp_NumberPickerView);
        int n = a.getIndexCount();
        for (int i = 0; i < n; i++) {
            int attr = a.getIndex(i);
            if (attr == R.styleable.cp_NumberPickerView_cp_npv_ShownCount) {
                mShownCount = a.getInt(attr, DEFAULT_SHOWN_COUNT);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_DividerColor) {
                mDividerColor = a.getColor(attr, DEFAULT_DIVIDER_COLOR);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_TextBgColorSelected) {
                mTextBgColorSelected = a.getColor(attr, DEFAULT_BG_SELECTED_COLOR);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_DividerHeight) {
                mDividerHeight = a.getDimensionPixelSize(attr, DEFAULT_DIVIDER_HEIGHT);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_DividerMarginLeft) {
                mDividerMarginL = a.getDimensionPixelSize(attr, DEFAULT_DIVIDER_MARGIN_HORIZONTAL);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_DividerMarginRight) {
                mDividerMarginR = a.getDimensionPixelSize(attr, DEFAULT_DIVIDER_MARGIN_HORIZONTAL);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_TextArray) {
                mDisplayedValues = convertCharSequenceArrayToStringArray(a.getTextArray(attr));
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_TextColorNormal) {
                mTextColorNormal = a.getColor(attr, DEFAULT_TEXT_COLOR_NORMAL);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_TextColorSelected) {
                mTextColorSelected = a.getColor(attr, DEFAULT_TEXT_COLOR_SELECTED);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_TextColorHint) {
                mTextColorHint = a.getColor(attr, DEFAULT_TEXT_COLOR_SELECTED);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_TextSizeNormal) {
                mTextSizeNormal = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_NORMAL_SP));
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_TextSizeSelected) {
                mTextSizeSelected = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_SELECTED_SP));
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_TextSizeHint) {
                mTextSizeHint = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_HINT_SP));
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_MinValue) {
                mMinShowIndex = a.getInteger(attr, 0);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_MaxValue) {
                mMaxShowIndex = a.getInteger(attr, 0);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_WrapSelectorWheel) {
                mWrapSelectorWheel = a.getBoolean(attr, DEFAULT_WRAP_SELECTOR_WHEEL);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_ShowDivider) {
                mShowDivider = a.getBoolean(attr, DEFAULT_SHOW_DIVIDER);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_ShowTextBgSelected) {
                mShowTextBgSelected = a.getBoolean(attr, DEFAULT_SHOW_TEXT_BG_SELECTED);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_HintText) {
                mHintText = a.getString(attr);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_AlternativeHint) {
                mAlterHint = a.getString(attr);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_EmptyItemHint) {
                mEmptyItemHint = a.getString(attr);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_MarginStartOfHint) {
                mMarginStartOfHint = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_MARGIN_START_OF_HINT_DP));
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_MarginEndOfHint) {
                mMarginEndOfHint = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_MARGIN_END_OF_HINT_DP));
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_ItemPaddingVertical) {
                mItemPaddingVertical = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_ITEM_PADDING_DP_V));
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_ItemPaddingHorizontal) {
                mItemPaddingHorizontal = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_ITEM_PADDING_DP_H));
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_AlternativeTextArrayWithMeasureHint) {
                mAlterTextArrayWithMeasureHint = a.getTextArray(attr);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_AlternativeTextArrayWithoutMeasureHint) {
                mAlterTextArrayWithoutMeasureHint = a.getTextArray(attr);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_RespondChangeOnDetached) {
                mRespondChangeOnDetach = a.getBoolean(attr, DEFAULT_RESPOND_CHANGE_ON_DETACH);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_RespondChangeInMainThread) {
                mRespondChangeInMainThread = a.getBoolean(attr, DEFAULT_RESPOND_CHANGE_IN_MAIN_THREAD);
            } else if (attr == R.styleable.cp_NumberPickerView_cp_npv_TextEllipsize) {
    ...
    private void init(Context context) {
        ...
        mPaintTextSelectedBg.setColor(mTextBgColorSelected);
        mPaintTextSelectedBg.setAntiAlias(true);
    ...
    public void setmShowDivider(boolean showDivider) {
        if (mShowDivider == showDivider) {
            return;
        }
    ...
    public void setmShowTextBgSelected(boolean showTextBgSelected) {
        if (mShowTextBgSelected == showTextBgSelected) {
            return;
        }
        mShowTextBgSelected = showTextBgSelected;
        postInvalidate();
    }

    public void setmTextBgColorSelected(int textBgColorSelected) {
        if (mTextBgColorSelected == textBgColorSelected) {
            return;
        }
        mTextBgColorSelected = textBgColorSelected;
        mPaintTextSelectedBg.setColor(mTextBgColorSelected);
        postInvalidate();
    }

    public void setmShownCount(int shownCount) {
        if (mShownCount == shownCount) {
            return;
        }
    ...
    private void drawCenterRect(Canvas canvas) {
        if (mShowTextBgSelected) {
            canvas.drawRect(getPaddingLeft() + mDividerMarginL, dividerY0, mViewWidth - getPaddingRight() - mDividerMarginR, dividerY1, mPaintTextSelectedBg);//-lee
        }
    }
