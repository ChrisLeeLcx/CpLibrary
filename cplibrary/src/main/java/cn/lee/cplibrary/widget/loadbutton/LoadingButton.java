...
public class LoadingButton extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
//            int defaultColor = Color.BLUE;//自定义修改颜色   0xFB9700
//             int RED         = 0xFFFF0000; RGBA   #0097FB
            int defaultColor = 0xFF0097FB;
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.cp_LoadingButton, 0, 0);
            mColorPrimary = ta.getInt(R.styleable.cp_LoadingButton_cp_lb_btnColor, defaultColor);
            String text = ta.getString(R.styleable.cp_LoadingButton_cp_lb_btnText);
            mText = text == null ? "" : text;
            mTextColor = ta.getColor(R.styleable.cp_LoadingButton_cp_lb_textColor, Color.WHITE);
            resetAfterFailed = ta.getBoolean(R.styleable.cp_LoadingButton_cp_lb_resetAfterFailed, true);
            mRippleColor = ta.getColor(R.styleable.cp_LoadingButton_cp_lb_btnRippleColor, Color.BLACK);
            mRippleAlpha = ta.getFloat(R.styleable.cp_LoadingButton_cp_lb_btnRippleAlpha, 0.3f);
            ta.recycle();
        }
