...
public final class ChineseCalendar extends GregorianCalendar {
    ...
    public static final int CHINESE_ZODIAC = 808;//属相
    ...
    private int hour = Calendar.getInstance().get(Calendar.HOUR_OF_DAY);//阴历阳历通用 0~23
    ...
    public String getGregorianHour() {
        return hour < 0 ? GLUtil.UNKNOWN_HOUR_LABEL : String.valueOf(hour) + "时";
    }

    public String getLunarHour() {
        return GLUtil.lunarHours[hour + 1];
    }

    /**
     * 选择的公历日期固定格式文本
     *
     * @return eg：公历：2019年11月28日 时辰未知 ，公历：2019年11月28日 0子时
     */
    public String getGregorianPickedTxt() {
        StringBuffer buf = new StringBuffer();
        buf.append("公历：").append(get(YEAR)).append("年").append(get(MONTH) + 1).append("月").append(get(DATE)).append("日 ")
                .append(hour < 0 ? "时辰" + GLUtil.UNKNOWN_HOUR_LABEL : getGregorianHour());
        return buf.toString();
    }
    /**
     * 选择的公历日期固定格式文本
     ...
     * @return eg：公历：2019年11月28日 时辰未知 ，公历：2019年11月28日 0子时
     */
    public String getCalendarTxt(Calendar c) {
        if(c==null){
            c = Calendar.getInstance();
        }
        int initYear = c.get(Calendar.YEAR);
        int initMonth = c.get(Calendar.MONTH) + 1;// 通过Calendar算出的月数要+1
        int initDate = c.get(Calendar.DATE);
        int initHour = c.get(Calendar.HOUR_OF_DAY);
        StringBuffer buf = new StringBuffer();
        buf.append("公历：").append(initYear).append("年").append(initMonth).append("月").append(initDate).append("日 ")
                .append(initHour+"时");
        return buf.toString();
    }
    /**
     * 选择的农历日期固定格式文本
     *
     * @return eg：农历：2019年十一月初十 时辰未知 ，农历：2019年十一月初十 3寅时
     */
    @SuppressLint("WrongConstant")
    public String getLunarPickedTxt() {
        StringBuffer buf = new StringBuffer();
        buf.append("农历：").append(get(ChineseCalendar.CHINESE_YEAR)).append("年").append(getChinese(CHINESE_MONTH)).append(getChinese(CHINESE_DATE))
                .append(hour < 0 ? " 时辰" + GLUtil.UNKNOWN_HOUR_LABEL : " "+getLunarHour());
        return buf.toString();
    }
     ...
     * 选择的公历日期固定格式文本
     *
     * @return eg：公历：2019-11-28 00  或者 2019-11-28
     */
    @SuppressLint("WrongConstant")
    public String getPickedFormat(boolean isGregorian) {
        String time = "";
        ...
        if (isGregorian) {//阳历
            ...
            year = get(ChineseCalendar.CHINESE_YEAR);
            month = get(ChineseCalendar.CHINESE_MONTH);
            day = get(ChineseCalendar.CHINESE_DATE);
        }
        time = hour < 0 ? String.format(Locale.CHINA, "%04d-%02d-%02d", year, month, day)
                : String.format(Locale.CHINA, "%04d-%02d-%02d %02d", year, month, day, hour);
        return time;
    }
}